        ...
        <div class="col min-vh-100 py-3">
            <!-- toggler -->
				...
				<div class="container">
						 <div class="row">
						    <div class="col-8">
							<button type="button" class="btn btn-success"><a style ="color :white"href = "/GestionDeClasse/newOccuper?action=new"><i class="bi-plus fs-5" style="font-weight: bold"></i></a></button>
							</div>
						 </div><br>	
								...
								<c:forEach items="${ Occupes }" var="occ">	
									<tr>
										<td><c:out value="${ occ.codeprof }"/></td>
									    <td><c:out value="${ occ.codesalle }"/></td>
										<td><c:out value="${ occ.thedate}"/></td>	
										<td>
						 					<button type="button" class="btn btn-success"><a style="color: white" href="/GestionDeClasse/occuper?action=modifier&codeprof='<c:out value='${ occ.codeprof }'/>'&codesalle='<c:out value='${ occ.codesalle }'/>'"><i class="bi-pencil-fill"></i></a></button>       
											<button type="button" class="btn btn-danger" onclick="return confirm('Êtes-vous sûr de vouloir supprimer?')"><a style="color: white" href="/GestionDeClasse/occuper?action=supprimer&codeprof=<c:out value='${occ.codeprof }' />&codesalle=<c:out value='${ occ.codesalle }' />"><i class="bi-trash"></i></a></button>             	
						                </td>
									</tr>
								</c:forEach>
								</table>
